"""Select node: pick the lowest-MAE model and set the confidence flag.
...
def select_best_model(state: AgentState) -> dict:
    scored = {
        k: v["mae"] for k, v in state["results"].items() if v.get("mae") is not None
    }
    if not scored:
        logger.warning(
            "Model eval [%s]: no model produced a backtest MAE — flagging low confidence",
            state.get("view", "?"),
        )
        return {"best_model": None, "confidence_flag": True}

    best = min(scored, key=scored.get)
    threshold = state.get("mae_confidence_threshold")
    if threshold is None:
        threshold = config.MAE_CONFIDENCE_THRESHOLD
    flag = threshold is not None and scored[best] > threshold

    table = " | ".join(f"{k}: MAE {v:.2f}" for k, v in sorted(scored.items()))
    logger.info(
