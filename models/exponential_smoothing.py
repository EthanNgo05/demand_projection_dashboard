...
RAW_DATA_FOLDER = "raw_data/demand_projections"
INPUT_GLOB = os.path.join(RAW_DATA_FOLDER, "all_demand_projections_*.xlsx")

# List-price workbook (SKU -> List Price USD), used to value the change in
# forecast as a revenue risk. See load_list_prices().
LIST_PRICE_GLOB = os.path.join("raw_data", "list_prices_*.xlsx")
...
def resolve_input_file():
    ...
    if not candidates:
        raise FileNotFoundError(
            ...
            f"Expected e.g. {RAW_DATA_FOLDER}/all_demand_projections_YYYY-MM-DD.xlsx"
        )
