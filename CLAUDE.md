...
- **`compute.py`** — the forecasting compute core: `list_views`, `compute_view`, `compute_by_customer`, `compute_by_customer_best` (best-model-per-group), agent-summary readers, Excel exporters.
...
- **`exceptions.py`** — the Exceptions view: `compute_exceptions` (8-week POS/Orders run-rate vs. the same 15 forward weeks the models use → gap/pct/USD impact) + `render_exceptions` with per-SKU detail cards.
