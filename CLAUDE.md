...
- **~57 raw customers** (`Custnmbr`), consolidated into a smaller set of **Customer Groupings** across 5 fulfillment regions: US (LBC+NJ), EU (SH-CTS), AU (ACR), CA (**18WHEELS** since Aug 2026, replacing YYZ5), JP (NETDEPOT). The warehouse→region mapping lives in ONE place — `REGION_WAREHOUSES` in `extract_warehouse_projections.py` — and `transform_to_regions` **fails the whole five-region pull** on a warehouse code it doesn't know, which is how the 18WHEELS cutover was caught. The models' `region_for_group` still returns the display label `"CA (YYZ5)"` on purpose: it is the view id behind the forecast cache and the `agent_summary_<view>.json` filenames, and nothing routes on the warehouse text (`data_io._region_code` reads only the leading two letters). Several raw customers fold into one group — e.g. AMAZON-DC / AMAZON-DS / MARVAL-FBM all become `AMAZON-DC`; Web + Warranty US become one group. Customers with no explicit mapping keep their own name as a single-member group, and region "Others - <country></country>" buckets catch the long tail.
...
# Interactive dashboard (localhost only)
streamlit run src/dashboard.py

# Interactive dashboard, served to the team on the LAN (what the shared instance runs)
...
# Nightly scheduler daemon: full DW sync + all models for every view, at 00:00
python src/scheduler.py                       # daemon (start via ./start_scheduler.ps1 at boot)
python src/scheduler.py --once                # run tonight's job now; exits with the worst step's code
python src/scheduler.py --once --dry-run      # print the plan + resolved lock/log paths, touch nothing
                                              # flags: --hour/--minute, --no-llm, --workers N, --force

# Warehouse pulls (or ./refresh_demand_data.ps1 to run all four steps by hand, without locks)
...
Quick Projections and Optimized Projections deliberately share one page shape — KPI row → total weekly demand → `Customer detail` → `SKU detail` → a condensed `Summary table by SKU and customer` whose rows expand into a detail card. Three pieces are literally shared, not merely parallel: **`kpis.render_sku_detail_section`** (the whole `SKU detail` section), **`kpis.render_sku_detail_card`** (the per-row card), and the **fixed filter bar** above both tables (`tables.FIXED_FILTER_LABELS`). `SKU detail`'s customer-group donut (`charts.customer_share_donut`) renders **inside** that section's chart column, under the SKU chart and beside its KPI tiles — not full width below the column row, where a 460px figure across the whole page reads as a gap rather than as part of the section.

`render_sku_detail_section` takes its KPI frame and its breakdown frame **at whatever grain the view has**: Quick passes the SKU-grain `summary` plus `by_cust`, Optimized passes `combined` (SKU × customer) for both. `_render_kpis` counts SKUs with `nunique` and sums the projection columns, so `combined` scoped to one SKU already *is* that SKU's roll-up — which is why **Optimized must not grow a `roll_up_summary` call of its own.** A second path to the same number is how two numbers start. The one thing that genuinely differs is `Data Source`, which `roll_up_summary` has already collapsed to `MIXED_SOURCE` on Quick's frame and which `_sku_detail_source` has to reduce itself on Optimized's.

Quick's two tables are ordered deliberately, pinned by `test_quick_default_view_is_all_customers_in_the_new_order`. The view-level per-SKU table (`Summary table by SKU (view total)` — the same numbers rolled up to the grain an order is placed at, plus the one column unique to it, `Top Volume Customer Groups`) comes **first**, directly under `SKU detail` because it is the table form of what that section charts. It is the only one in an expander, collapsed, so it is titled by its label rather than an `###` heading (tests for it must look at `at.expander`, not `at.markdown`). The by-customer table follows it as a plain always-visible `###` section — it is the page's main content and is never folded away. It is narrowed by the **fixed filter bar** (see `tables.py` below), not by a picker of its own: it used to carry a standalone `by_cust_sku` dropdown above the filter chips, and the bar's SKU filter is that control, so there is one instead of two. **The bar narrows the DISPLAY only: `dl_by_customer` exports `by_cust_table`, the full frame, and must keep doing so** — a silently narrowed workbook is indistinguishable from a complete one once it is off the page.
...
- **`charts.py`** / **`tables.py`** / **`kpis.py`** — Plotly builders + date-range control; summary-table styling and the table filters (`render_filtered_table` / `render_selectable_table`); the KPI row, the shared `SKU detail` section and the Optimized (best-model-combined) render.

**Table filters: two layouts, one field/mask model.** `tables.filter_table` builds a whitelist of fields (`_build_fields`) and AND-s their masks, with Excel cross-filtering — `available()` narrows each dropdown to the values that still yield rows under every *other* field's pick, so Region = AU leaves no AMEA-only customer selectable. It renders them one of two ways:

- **Add-filter chips** (default; Exceptions, Spikes, Watchlist, the by-SKU view-total expander, data quality). A `➕ Add filter…` picker activates one of nine fields; each shows as a row with a `✕`. Fields only appear in the menu when they'd actually narrow something.
- **A fixed bar** — `fixed=FIXED_FILTER_LABELS`, used by *both* main projections tables. SKU / Customer / Region as dropdowns side by side, `⭐ Key SKUs only` as a pressable button beneath. `fixed` also **relaxes the "would this narrow?" gates**: those exist to keep the add-filter *menu* short, and a control the caller promised is always on screen must not vanish on a single-region view. The exception is Key SKU's `.any()` — a button that can only empty the table is worse than no button.

Two behaviours ride on the bar. Every control clears `{key}__sel` (`_clear_row_selection`), because `render_selectable_table` keys its selection **positionally** and a narrowed frame would otherwise leave an open card describing a different (SKU, customer) than the row that opened it. And `sku_filter_narrowed` derives `focused` — one SKU picked leaving one row drops the table and opens that row's card outright, which is what the callers' old `focus_single` argument did by hand.
...
- **`refresh.py`** — subprocess-backed manual refresh (demand / warehouse / key-SKUs / agent batch) with lock/log files and progress polling. The lock/failure-marker/child-process primitives themselves live in **`src/joblocks.py`** *(streamlit-free)* and are re-exported here under their original private names, because `src/scheduler.py`'s nightly daemon writes the same files and cannot import this module.
...
Within one view the models fit **serially** — Holt-Winters dominates the runtime, so per-model parallelism doesn't pay. Instead `agent.batch` fans the ~114 views across a `ProcessPoolExecutor` of **single-threaded** workers: thread-cap env vars (`OMP_NUM_THREADS` etc., `XGB_N_JOBS=1`) are set in the parent *before* the pool spawns, so workers import NumPy/XGBoost single-threaded and N workers use N cores without contention. The parent ingests once (snapshot read + Plytix fetch) and hands every worker the cleaned frame via one temp Parquet file — `ingest` short-circuits when the state already carries `cleaned_df`, so no worker re-reads or re-fetches.

### Data flow & inputs

- **Nightly job** (`src/scheduler.py`, an APScheduler daemon firing at 00:00 local): full demand pull → warehouse pull (independent — runs even if the demand pull failed) → key-SKUs pull (`extract_key_skus.py`) → `agent.batch` precompute (only if the demand pull succeeded). The nightly demand pull is deliberately the **full 36-month pull** — the self-healing baseline that picks up restated actuals, item renames, and customer remaps; the dashboard's refresh button runs the fast `--incremental` pull instead. Worst exit code wins. Runs ~00:00-01:20 (the batch alone is ~50-60 min). The batch step also **warms `forecast_cache`** (and prunes it), so the first person in each morning reads forecasts off disk instead of recomputing. Warm hits require the batch's `today_ts` (wall clock) to match the snapshot's date label, which the pull-then-batch ordering gives; if they diverge the dashboard simply recomputes and writes its own entry — a miss, never a wrong number.
  - **The daemon writes the files the dashboard already reads**, which is the point of doing this in Python: every step takes the same `.refresh.lock` the refresh buttons take (so a click at 00:05 and the nightly run cannot both launch a ten-minute query - the loser *skips*, it does not fail), writes the same `===== <label> started <ts> on <host> =====` run header into the same `logs/<date>/logs_refresh_*.txt`, and on a non-zero exit leaves the same `<lock>.failed` marker that drives the dashboard's red banner. A bad night is therefore visible by morning without anyone opening a log - the failure mode that once left the app serving six-day-old data. Those primitives live in **`src/joblocks.py`** (Streamlit-free) precisely because `dashboard_app/refresh.py` imports `streamlit` and reads `st.session_state`, so the daemon cannot import it; `refresh.py` re-exports them under their original private names.
  - **`refresh_demand_data.ps1` is now the manual/one-shot wrapper only** - it runs the same four steps synchronously but takes no locks and writes no failure markers. It was documented as registered with Task Scheduler and never was (no `===== scheduled DW refresh` header appears anywhere in `logs/`), which is why all data arrived by button click until 2026-08-18. `start_scheduler.ps1` is the boot wrapper for the daemon; register **that** with Task Scheduler on an `onstart` trigger. The 00:00 time itself lives in `scheduler.py` (`--hour`/`--minute`, or `DEMAND_SCHEDULE_HOUR`/`DEMAND_SCHEDULE_MINUTE`), not in Task Scheduler.
- `sql/demand_details_optimized.sql` is the **default** query behind the demand extract (`DEFAULT_SQL` in `extract_demand_details.py`); `--incremental` only works against it (it rewrites a marker line in the batch to narrow the date window). The legacy `sql/demand_details.sql` is **UTF-16 encoded** (opens as garbled/spaced text in some tools — that's expected, per `.gitattributes`). Region "Others - <country></country>" buckets attach at `Custnmbr` grain via `MIN(Customer)` — don't drop them when touching the SQL.
