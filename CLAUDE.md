...
Quick puts **both** its tables in collapsed expanders, titled by the expander label rather than an `###` heading (so `_headings`-style tests must look at `at.expander`, not `at.markdown`). The by-customer one carries its own SKU dropdown (`by_cust_sku`), whose options are `ALL_SKUS_OPTION` — a UI-only sentinel, never a SKU value — followed by the table's own SKUs; changing it clears the table's positional row selection via `dashboard._clear_by_cust_selection`. **The picker and the filter chips narrow the DISPLAY only: `dl_by_customer` exports `by_cust_table`, the full frame, and must keep doing so** — a silently narrowed workbook is indistinguishable from a complete one once it is off the page. Below it sits a view-level per-SKU table in its own expander: the same numbers rolled up to the grain an order is placed at, plus the one column unique to it (`Top Volume Customer Groups`).
