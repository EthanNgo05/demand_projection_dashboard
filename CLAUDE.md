...
Quick Projections and Optimized Projections deliberately share one page shape — KPI row → total weekly demand → `Customer detail` → `SKU detail` → a condensed `Summary table by SKU and customer` whose rows expand into a detail card (`kpis.render_sku_detail_card`, shared by both). `SKU detail`'s customer-group donut (`charts.customer_share_donut`) renders **inside** that section's chart column, under the SKU chart and beside its KPI tiles — not full width below the column row, where a 460px figure across the whole page reads as a gap rather than as part of the section.

Quick's two tables are ordered deliberately, pinned by `test_quick_default_view_is_all_customers_in_the_new_order`. The view-level per-SKU table (`Summary table by SKU (view total)` — the same numbers rolled up to the grain an order is placed at, plus the one column unique to it, `Top Volume Customer Groups`) comes **first**, directly under `SKU detail` because it is the table form of what that section charts. It is the only one in an expander, collapsed, so it is titled by its label rather than an `###` heading (tests for it must look at `at.expander`, not `at.markdown`). The by-customer table follows it as a plain always-visible `###` section — it is the page's main content and is never folded away. It carries its own SKU dropdown (`by_cust_sku`), whose options are `ALL_SKUS_OPTION` — a UI-only sentinel, never a SKU value — followed by the table's own SKUs; changing it clears the table's positional row selection via `dashboard._clear_by_cust_selection`. **The picker and the filter chips narrow the DISPLAY only: `dl_by_customer` exports `by_cust_table`, the full frame, and must keep doing so** — a silently narrowed workbook is indistinguishable from a complete one once it is off the page.
